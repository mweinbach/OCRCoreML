...
public enum OCRDetectorError: Error, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case .missingBundledModel:
            "Missing bundled Detector224.mlpackage resource"
        ...
        case .invalidInputShape(let expected, let actual):
            "Invalid image input shape. Expected \(expected), got \(actual)"
...
public final class OCRDetector {
    ...
    public static var bundledModelURL: URL {
        get throws {
            guard let url = Bundle.module.url(forResource: "Detector224", withExtension: "mlpackage") else {
                throw OCRDetectorError.missingBundledModel
            }
            return url
        }
    }
    ...
    public init(modelURL: URL? = nil, computeUnits: MLComputeUnits = .all) throws {
        ...
        self.model = try Self.loadModel(at: resolvedModelURL, configuration: configuration)
    }
    ...
    public func prediction(input imageArray: MLMultiArray) throws -> OCRDetectorPrediction {
        try Self.validateInputShape(imageArray)
    ...
    private static func validateInputShape(_ array: MLMultiArray) throws {
        let actual = array.shape.map(\.intValue)
        guard actual == inputShape else {
            throw OCRDetectorError.invalidInputShape(expected: inputShape, actual: actual)
        }
    }
}
